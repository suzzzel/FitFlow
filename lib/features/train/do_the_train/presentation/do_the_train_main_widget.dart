...
class _DoTheTrainMainWidgetState extends ConsumerState<DoTheTrainMainWidget> {
  ...
  void _next({required int tempExercise}) {
    setState(() {
      ref
  ...
  Widget build(BuildContext context) {
    ...
    AsyncValue<ExerciseModel> tempExercise = ref.watch(
        tempExerciseFutureProvider(getTempEx(
            tempExercise: trainNotifier.tempExercise, train: trainNotifier)));
      ...
      children: [
        dir.when(
            data: (gifDirectory) {
              final exGifID = getTempEx(
                      ...
                      train: trainNotifier)
                  .padLeft(4, '0');
              final gifEx = File('${gifDirectory.path}/exGifs/$exGifID.gif');
              return Column(
                children: [
                  SizedBox(
                        ...
                        return Column(
                          children: [
                            ...
                            Container(
                              ...
                              child: Text('Главная мышца: ${exercise.target}'),
                            ),
                            Container(
                              color: Colors.yellow,
                              child: Row(
                              ...
                              children: [
                                ExpansionPanel(
                                    ...
                                    headerBuilder: (context, isOpen) {
                                      return Text('Инструкция к выполнению');
                                    },
                                    body: Column(
                                      children: List.generate(
                                          stepOfInstructionsOpen.length,
                                          (int index) {
                                        return ExpansionPanelList(
                                          expansionCallback:
                                              (panelIndex, isExpanded) {
                                            setState(() {
                                              stepOfInstructionsOpen[index] =
                                                  isExpanded;
                                            });
                                          },
                                          children: [
                                            ExpansionPanel(
                                                ...
                                                headerBuilder:
                                                    (context, isOpen) {
                                                  return Text('Шаг №$index');
                                                },
                                                body: Text(instructions[index]))
                                          ],
                                        );
                                      }),
                                    )),
        ...
        ElevatedButton(
            onPressed: () {
              ...
              _next(tempExercise: trainNotifier.tempExercise);
            },
            child: Text('press')),
        ElevatedButton(
            onPressed: () {
